...
*/
var addFileItem = function (link, originName, format) {
  ...
  item.setAttribute('data-file', originName + "." + format);
  item.setAttribute('data-url', link);

  item.innerHTML = /*html*/`
    <i class="fa fa-file-text-o" aria-hidden="true"></i>
    <span>${originName}.${format}</span>
  `;
...
var widget = cloudinary.createUploadWidget(config, (error, result) => {
  ...
  if (result && result.event === "success") {
    addFileItem(result.info.secure_url, result.info.original_filename, result.info.format);
  }
});
